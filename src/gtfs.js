...
function GTFS(path, startDate) {
	...
	// init stop_times
	raw_data.stop_times.forEach(st => {
	...
	// now clean up
	raw_data.calendar.forEach(c => { throw Error() });

	raw_data.calendar_dates.forEach(d => {
	...
	data.trips.forEach(function (trip) {
		trip.route = data.routes[trip.route];
		trip.service = data.services[trip.service];
		trip.dates = trip.service.dates;
		trip.stops = trip.stops.map(function (stop_index, index) {
			var stop = data.stops[stop_index];
